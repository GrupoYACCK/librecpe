...
data = {
    "tipo_de_comprobante": {"01": "1", "03": "2", "07": "3", "08": "4", "09": "7"},
    "sunat_transaction": {
        "0101": "1",
        "0200": "2",
        "0101": "4",
    ...
    "moneda": {"PEN": "1", "USD": "2", "EUR": "3"},
    "percepcion_tipo": {"01": "1", "02": "2", "03": "3"},
    "retencion_tipo": {
        "01": "1",
        "02": "2",
    },
    ...
    "tipo_de_nota_de_debito": {"01": "1", "02": "2", "03": "3", "11": "5", "12": "4"},
    "tipo_de_igv": {
        "10": "1",
        "11": "2",
        "12": "3",
...
class NubeFactPSE:
    ...
    def _getCliente(self):
        vals = {}
        cliente = (
            self.documento.tipoDocumento == "09"
            and self.documento.remitente
            or self.documento.adquirente
        )
    ...
    def _getDocumentoModificado(self):
        ...
        for documentoModificado in self.documento.documentosModificados:
            ...
            vals["documento_que_se_modifica_serie"] = documentoModificado.numero.split(
                "-"
            )[0]
            vals["documento_que_se_modifica_numero"] = documentoModificado.numero.split(
                "-"
            )[-1]
    ...
    def _getTotales(self):
        ...
        # total_anticipo = 0.0 self.documento.totalAnticipos -
        # sum(detalle.impuestos for detalle in self.documento.anticipos)
    ...
    def _getItems(self):
        ...
        for detalle in self.documento.detalles:
            ...
            vals["valor_unitario"] = (
                detalle.mtoValorUnitario or detalle.mtoValorReferencialUnitario
            )
            vals["precio_unitario"] = (
                detalle.mtoPrecioVentaUnitario or detalle.mtoValorReferencialUnitario
            )
            descuento = 0.0
            if self.documento.tipoDocumento not in ["07", "08"]:
                for desc in detalle.cargoDescuentos:
            ...
            for tributo in detalle.tributos:
                if (
                    descuento > 0.0
                    and tributo.procentaje > 0.0
                    and tributo.ideTributo != "7152"
                ):
                    vals["precio_unitario"] = round(
                        vals["valor_unitario"] * (1 + tributo.procentaje / 100), 10
                    )
            ...
            vals["tipo_de_igv"] = data["tipo_de_igv"].get(
                detalle.tipAfectacion, detalle.tipAfectacion
            )
            vals["igv"] = igv
        ...
        for detalle in self.documento.anticipos:
            ...
            for tributo in detalle.tributos:
                if tributo.ideTributo == "7152":
                    vals["precio_unitario"] = round(
                        vals["valor_unitario"] + tributo.montoTributo, 10
                    )
                    total_impuestos_bolsas += tributo.montoTributo
                elif tributo.ideTributo == "1000":
                    igv += tributo.montoTributo
            ...
            vals["valor_unitario"] = round(
                detalle.monto - igv - total_impuestos_bolsas, 2
            )
    ...
    def _getGuias(self):
        ...
        for guia in self.documento.guias:
            res.append(
                {
                    "guia_tipo": data["guia_tipo"].get(guia.tipoDocumento, "1"),
    ...
    def _getVentaCredito(self):
        ...
        for cuota in self.documento.medioPago.cuotas:
            res.append(
                {
                    "cuota": num,
                    "fecha_de_pago": datetime.strptime(
                        cuota.fecha, "%Y-%m-%d"
                    ).strftime("%d-%m-%Y"),
                    "importe": cuota.monto,
                }
    ...
    def _getDetraccion(self):
        ...
        if self.documento.detraccion:
            ...
            vals["detraccion_tipo"] = data.get("detraccion_tipo", {}).get(
                self.documento.detraccion.codigo
            )
    ...
    def _getRetencion(self):
        ...
        if self.documento.retencion:
            vals["retencion_tipo"] = data.get("retencion_tipo").get(
                self.documento.retencion.codigo
            )
    ...
    def getDocumento(self):
        ...
        vals["tipo_de_comprobante"] = data["tipo_de_comprobante"].get(
            self.documento.tipoDocumento, self.documento.tipoDocumento
        )
        vals["serie"] = self.documento.numero.split("-")[0]
        vals["numero"] = self.documento.numero.split("-")[1]
        vals["sunat_transaction"] = data["sunat_transaction"].get(
            self.documento.tipOperacion, self.documento.tipOperacion
        )
        vals["fecha_de_emision"] = self.documento.fecEmision.strftime("%d-%m-%Y")
        vals["fecha_de_vencimiento"] = (
            self.documento.fecVencimiento
            and datetime.strptime(self.documento.fecVencimiento, "%Y-%m-%d").strftime(
                "%d-%m-%Y"
            )
            or ""
        )
        vals["moneda"] = data["moneda"].get(
            self.documento.tipMoneda, self.documento.tipMoneda
        )
        # "tipo_de_cambio": "",
        ...
        if self.documento.medioPago.cuotas:
            ...
            vals["medio_de_pago"] = ""
        ref = (
            self.documento.referencia.replace(" ", "").replace("-", "").replace("/", "")
        )
    ...
    def anularDocumento(self, servidor):
        ...
        for anulado in self.documento.documentosAnulados:
            vals = {
                ...
                "tipo_de_comprobante": data["tipo_de_comprobante"].get(
