...
class Ubl20:
    ...
    def _nsmap(self, documento):
        nsmap = OrderedDict(
            [
                (
                    None,
                    "urn:sunat:names:specification:ubl:peru:schema:xsd::%s" % documento,
                ),
                ("cac", self._cac),
                ("cbc", self._cbc),
                ("ds", self._ds),
    ...
    def _getEmisor(self):
        ...
        etree.SubElement(supplier, tag.text, nsmap={"cbc": tag.namespace}).text = (
            emisor.numDocumento
        )
    ...
    def _getAdquirente(self, detalle=None, adquirente=None):
        ...
        etree.SubElement(customer, tag.text, nsmap={"cbc": tag.namespace}).text = (
            adquirente.numDocumento or "0"
        )
        tag = etree.QName(self._cbc, "AdditionalAccountID")
        etree.SubElement(customer, tag.text, nsmap={"cbc": tag.namespace}).text = (
            adquirente.tipoDocumento or "-"
        )
    ...
    def _getRecepetor(self):
        ...
        identification = etree.SubElement(
            supplier, tag.text, nsmap={"cac": tag.namespace}
        )
        tag = etree.QName(self._cbc, "ID")
        etree.SubElement(
            identification,
        ...
        tag = etree.QName(self._cbc, "Name")
        etree.SubElement(party_name, tag.text, nsmap={"cbc": tag.namespace}).text = (
            etree.CDATA(adquirente.nomComercial)
        )
        ...
        etree.SubElement(country, tag.text, nsmap={"cbc": tag.namespace}).text = (
            adquirente.codPais
        )
    ...
    def _getTributos(self, root, total, documento=None):
        for tributo in documento.tributos:
            ...
            etree.SubElement(
                total,
                tag.text,
                currencyID=documento.tipMoneda,
                nsmap={"cbc": tag.namespace},
            ...
            tag = etree.QName(self._cbc, "ID")
            etree.SubElement(scheme, tag.text, nsmap={"cbc": tag.namespace}).text = (
                tributo.ideTributo
            )
            tag = etree.QName(self._cbc, "Name")
            etree.SubElement(scheme, tag.text, nsmap={"cbc": tag.namespace}).text = (
            ...
            etree.SubElement(scheme, tag.text, nsmap={"cbc": tag.namespace}).text = (
                tributo.codTipTributo
            )

    def _getUbl(self, customization="1.0"):
        etree.SubElement(
            self._root, "{%s}%s" % (self._cbc, "UBLVersionID"), nsmap={"cbc": self._cbc}
        ).text = "2.0"
        etree.SubElement(
            self._root,
    ...
    def _getResumen(self):
        ...
        for documento in self.documento.documentos:
            ...
            etree.SubElement(line, tag.text, nsmap={"cbc": tag.namespace}).text = (
            ...
            etree.SubElement(status, tag.text, nsmap={"cbc": tag.namespace}).text = (
                documento.condicion or "1"
            )
    ...
    def _getAnulacion(self):
        ...
        for anulado in self.documento.documentosAnulados:
            ...
            etree.SubElement(line, tag.text, nsmap={"sac": tag.namespace}).text = (
                anulado.descripcion or "Anulado"
            )
    ...
    def _getDocumentosModificados(self, documentosModificados, detalle=False):
        for documentoModificado in documentosModificados:
            ...
            reference = etree.SubElement(
                detalle, tag.text, nsmap={"cac": tag.namespace}
            )
    ...
    def _getAgente(self):
        ...
        tag = etree.QName(self._cbc, "Name")
        etree.SubElement(party_name, tag.text, nsmap={"cbc": tag.namespace}).text = (
            etree.CDATA(emisor.nomComercial)
        )
    ...
    def _getDetalleRetencion(self):
        for detalle in self.documento.detalles:
            ...
            etree.SubElement(
                inv_line,
                tag.text,
                schemeID=detalle.tipoDocumento,
                nsmap={"cbc": tag.namespace},
            ).text = detalle.numero

            tag = etree.QName(self._cbc, "IssueDate")
            etree.SubElement(inv_line, tag.text, nsmap={"cbc": tag.namespace}).text = (
            ...
            etree.SubElement(
                inv_line,
                tag.text,
                currencyID=detalle.monedaFactura,
                nsmap={"cbc": tag.namespace},
            ).text = str(detalle.totalFactura)
            for pago in detalle.pagos:
                ...
                tag = etree.QName(self._cbc, "ID")
                etree.SubElement(
                    payment, tag.text, nsmap={"cbc": tag.namespace}
                ).text = str(pago.numeroPago)
                tag = etree.QName(self._cbc, "PaidAmount")
                etree.SubElement(
                    payment,
                    tag.text,
                    currencyID=pago.moneda,
                    nsmap={"cbc": tag.namespace},
                ).text = str(pago.monto)
            ...
            information = etree.SubElement(
                inv_line, tag.text, nsmap={"sac": tag.namespace}
            )
            tag = etree.QName(self._sac, "SUNATRetentionAmount")
            etree.SubElement(
                information,
                tag.text,
                currencyID=detalle.monedaRetencion,
                nsmap={"sac": tag.namespace},
            ).text = str(detalle.totalRetencion)
            tag = etree.QName(self._sac, "SUNATRetentionDate")
            etree.SubElement(
                information, tag.text, nsmap={"sac": tag.namespace}
    ...
    def _getRetencion(self):
        ...
        etree.SubElement(self._root, tag.text, nsmap={"sac": tag.namespace}).text = (
            self.documento.codigoRetencion
        )

        tag = etree.QName(self._sac, "SUNATRetentionPercent")
        etree.SubElement(self._root, tag.text, nsmap={"sac": tag.namespace}).text = str(
            self.documento.porcentajeRetencion
        )
        ...
        if self.documento.notas:
            ...
            ).text = etree.CDATA(self.documento.notas)
        ...
        etree.SubElement(
            ...
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.totalRetencion)
        ...
        etree.SubElement(
            ...
            currencyID=self.documento.tipMoneda,
            nsmap={"sac": tag.namespace},
        ).text = str(self.documento.totalNetoPagado)
