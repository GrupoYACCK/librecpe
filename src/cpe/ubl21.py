...
class Ubl21:
    ...
    def _getDocumento(self):
        etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'ID'),
                         nsmap={'cbc': self._cbc}).text = self.documento.numero or ''
        etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'IssueDate'),
                         nsmap={'cbc': self._cbc}).text = self.documento.fecEmision.strftime("%Y-%m-%d")
        etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'IssueTime'),
                         nsmap={'cbc': self._cbc}).text = self.documento.fecEmision.strftime("%H:%M:%S")
        if self.documento.tipoDocumento in ['01', '03']:
            if self.documento.fecVencimiento:
                etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'DueDate'),
                                 nsmap={'cbc': self._cbc}).text = self.documento.fecVencimiento or ''
            etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'InvoiceTypeCode'), listID=self.documento.tipOperacion,
                             listAgencyName="PE:SUNAT", listName="Tipo de Documento",
                             listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo01",
                             nsmap={'cbc': self._cbc}).text = self.documento.tipoDocumento
        if self.documento.tipoDocumento in ['09']:
            ...
            if self.documento.observacion:
                ...
                etree.SubElement(self._root, tag.text,
                                 nsmap={'cbc': tag.namespace}).text = self.documento.observacion or '-'
        # Leyenda
        if self.documento.tipoDocumento not in ['09']:
            for leyenda in self.documento.leyendas:
                tag = etree.QName(self._cbc, 'Note')
                etree.SubElement(self._root, tag.text, languageLocaleID=leyenda.codLeyenda,
                                 nsmap={'cbc': tag.namespace}).text = leyenda.desLeyenda

            etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'DocumentCurrencyCode'), listID='ISO 4217 Alpha',
                             listName='Currency', listAgencyName='United Nations Economic Commission for Europe',
            ...
            if self.documento.tipoDocumento in ['01', '03']:
                for guia in self.documento.guias:
                    despatch = etree.SubElement(self._root, "{%s}%s" % (self._cac, 'DespatchDocumentReference'),
                                                nsmap={'cac': self._cac})
                    etree.SubElement(despatch, "{%s}%s" % (self._cbc, 'ID'),
                                     nsmap={'cbc': self._cbc}).text = guia.numero
    ...
    def _getMedioPago(self):
        if (self.documento.tipoDocumento in [
            '07'] and self.documento.medioPago.tipo == 'Credito') or self.documento.tipoDocumento not in ['07']:
            payment = etree.SubElement(self._root, "{%s}%s" % (self._cac, 'PaymentTerms'), nsmap={'cac': self._cac})
            etree.SubElement(payment, "{%s}%s" % (self._cbc, 'ID'), nsmap={'cbc': self._cbc}).text = 'FormaPago'
            etree.SubElement(payment, "{%s}%s" % (self._cbc, 'PaymentMeansID'),
                             nsmap={'cbc': self._cbc}).text = self.documento.medioPago.tipo
        if self.documento.medioPago.tipo == 'Credito':
            ...
            for cuota in self.documento.medioPago.cuotas:
                ...
                etree.SubElement(payment, "{%s}%s" % (self._cbc, 'ID'), nsmap={'cbc': self._cbc}).text = 'FormaPago'
                etree.SubElement(payment, "{%s}%s" % (self._cbc, 'PaymentMeansID'),
                                 nsmap={'cbc': self._cbc}).text = "Cuota%03d" % i
                etree.SubElement(payment, "{%s}%s" % (self._cbc, 'Amount'), currencyID=self.documento.tipMoneda,
    ...
    def _getEmisor(self):
        ...
        if self.documento.tipoDocumento in ['09']:
            ...
            identification = etree.SubElement(party, "{%s}%s" % (self._cac, 'PartyIdentification'),
                                              nsmap={'cac': self._cac})
            etree.SubElement(identification, "{%s}%s" % (self._cbc, 'ID'), schemeID=emisor.tipoDocumento,
                             schemeName="Documento de Identidad",
                             schemeAgencyName="PE:SUNAT", schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06",
                             nsmap={'cbc': self._cbc}).text = emisor.numDocumento
            ...
            if emisor.nomComercial and emisor.nomComercial != '-':
                ...
                etree.SubElement(name, "{%s}%s" % (self._cbc, 'Name'), nsmap={'cbc': self._cbc}).text = etree.CDATA(
                    emisor.nomComercial)
            entity = etree.SubElement(party, "{%s}%s" % (self._cac, 'PartyLegalEntity'), nsmap={'cac': self._cac})
            etree.SubElement(entity, "{%s}%s" % (self._cbc, 'RegistrationName'),
                             nsmap={'cbc': self._cbc}).text = etree.CDATA(emisor.nombre)
            ...
            etree.SubElement(address, tag.text, schemeAgencyName='PE:INEI', schemeName='Ubigeos',
                             nsmap={'cbc': tag.namespace}).text = emisor.ubigeo

            etree.SubElement(address, "{%s}%s" % (self._cbc, 'AddressTypeCode'),
                             nsmap={'cbc': self._cbc}).text = emisor.codEstablecimiento

            tag = etree.QName(self._cbc, 'CitySubdivisionName')
            etree.SubElement(address, tag.text, nsmap={'cbc': tag.namespace}).text = etree.CDATA(
                emisor.urbanizacion or 'SN')
    ...
    def _getAdquirente(self, detalle=None, adquirente=None, tercero=None):
        if self.documento.tipoDocumento in ['09']:
            ...
            identification = etree.SubElement(party, "{%s}%s" % (self._cac, 'PartyIdentification'),
                                              nsmap={'cac': self._cac})
            etree.SubElement(identification, "{%s}%s" % (self._cbc, 'ID'), schemeID=destinatario.tipoDocumento,
                             schemeName="Documento de Identidad",
                             schemeAgencyName="PE:SUNAT", schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06",
                             nsmap={'cbc': self._cbc}).text = destinatario.numDocumento
            tag = etree.QName(self._cac, 'PartyLegalEntity')
            ...
            supplier = etree.SubElement(self._root, "{%s}%s" % (self._cac, 'AccountingCustomerParty'),
                                        nsmap={'cac': self._cac})
            party = etree.SubElement(supplier, "{%s}%s" % (self._cac, 'Party'), nsmap={'cac': self._cac})
            identification = etree.SubElement(party, "{%s}%s" % (self._cac, 'PartyIdentification'),
                                              nsmap={'cac': self._cac})
            etree.SubElement(identification, "{%s}%s" % (self._cbc, 'ID'), schemeID=adquirente.tipoDocumento,
                             schemeName="Documento de Identidad",
                             schemeAgencyName="PE:SUNAT", schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06",
                             nsmap={'cbc': self._cbc}).text = adquirente.numDocumento
            entity = etree.SubElement(party, "{%s}%s" % (self._cac, 'PartyLegalEntity'), nsmap={'cac': self._cac})
            etree.SubElement(entity, "{%s}%s" % (self._cbc, 'RegistrationName'),
                             nsmap={'cbc': self._cbc}).text = etree.CDATA(adquirente.nombre)
    ...
    def _getAnticipos(self):
        if self.documento.tipoDocumento in ['01', '03']:
            ...
            for anticipo in self.documento.anticipos:
                ...
                additional = etree.SubElement(self._root, tag.text, nsmap={'cac': tag.namespace})
                tag = etree.QName(self._cbc, 'ID')
                etree.SubElement(additional, tag.text, schemeID=self.documento.tipoDocumento,
                                 nsmap={'cbc': tag.namespace}).text = anticipo.numero

                tag = etree.QName(self._cbc, 'DocumentTypeCode')
                etree.SubElement(additional, tag.text, listName='Documento Relacionado', listAgencyName='PE:SUNAT',
                                 listURI='urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo12',
                                 nsmap={'cbc': tag.namespace}).text = anticipo.tipoCodigo
                ...
                tag = etree.QName(self._cbc, 'ID')
                etree.SubElement(party, tag.text, schemeID=self.documento.emisor.tipoDocumento,
                                 schemeName='Documento de Identidad',
                                 schemeAgencyName='PE:SUNAT',
                                 schemeURI='urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06',
                                 nsmap={'cbc': tag.namespace}).text = self.documento.emisor.numDocumento
    ...
    def _getMontoAnticipos(self):
        if self.documento.tipoDocumento in ['01', '03']:
            ...
            for anticipo in self.documento.anticipos:
                ...
                tag = etree.QName(self._cbc, 'ID')
                etree.SubElement(prepaid, tag.text, schemeName='Anticipo', schemeAgencyName='PE:SUNAT',
                                 nsmap={'cbc': tag.namespace}).text = str(i)  # line.move_name
                i += 1
                tag = etree.QName(self._cbc, 'PaidAmount')
                etree.SubElement(prepaid, tag.text, currencyID=self.documento.tipMoneda,
                                 nsmap={'cbc': tag.namespace}).text = str(anticipo.monto)
    ...
    def _getCargoDescuentos(self, root, cargoDescuentos, detalle=False):
        for cargoDescuento in cargoDescuentos:
            ...
            tag = etree.QName(self._cbc, 'AllowanceChargeReasonCode')
            if detalle:
                etree.SubElement(allowance_charge, tag.text,
            ...
            tag = etree.QName(self._cbc, 'Amount')
            etree.SubElement(allowance_charge, tag.text, currencyID=self.documento.tipMoneda,
                             nsmap={'cbc': tag.namespace}).text = str(cargoDescuento.monto)
    ...
    def _getTributos(self, root, total, detalle=None, tributos=None):
        ...
        etree.SubElement(tax_total, "{%s}%s" % (self._cbc, 'TaxAmount'), currencyID=self.documento.tipMoneda,
                         nsmap={'cbc': self._cbc}).text = str(round(total, 2))
        ...
        for tributo in tributos:
            ...
            if detalle and tributo.ideTributo not in ['7152']:
                etree.SubElement(tax_subtotal, "{%s}%s" % (self._cbc, 'TaxableAmount'),
                                 currencyID=self.documento.tipMoneda, nsmap={'cbc': self._cbc}).text = str(
                    round(tributo.baseTributo, 2))
            elif not detalle:
                if tributo.ideTributo not in ['7152']:
                    etree.SubElement(tax_subtotal, "{%s}%s" % (self._cbc, 'TaxableAmount'),
                                     currencyID=self.documento.tipMoneda, nsmap={'cbc': self._cbc}).text = str(
                        round(tributo.baseTributo, 2))
            etree.SubElement(tax_subtotal, "{%s}%s" % (self._cbc, 'TaxAmount'), currencyID=self.documento.tipMoneda,
                             nsmap={'cbc': self._cbc}).text = str(round(tributo.montoTributo, 2))
            if detalle and tributo.ideTributo in ['7152']:
                etree.SubElement(tax_subtotal, "{%s}%s" % (self._cbc, 'BaseUnitMeasure'), unitCode="NIU",
                                 nsmap={'cbc': self._cbc}).text = str(tributo.cantidad)
            tax_category = etree.SubElement(tax_subtotal, "{%s}%s" % (self._cac, 'TaxCategory'),
                                            nsmap={'cac': self._cac})
            if detalle:
                if tributo.ideTributo in ['7152']:
                    etree.SubElement(tax_category, "{%s}%s" % (self._cbc, 'Percent'),
                                     nsmap={'cbc': self._cbc}).text = str(0.0)
                    etree.SubElement(tax_category, "{%s}%s" % (self._cbc, 'PerUnitAmount'),
                                     currencyID=self.documento.tipMoneda, nsmap={'cbc': self._cbc}).text = str(
                        tributo.procentaje)
                else:
                    etree.SubElement(tax_category, "{%s}%s" % (self._cbc, 'Percent'),
                                     nsmap={'cbc': self._cbc}).text = str(tributo.procentaje)
                if tributo.ideTributo == "2000":
                    etree.SubElement(tax_category, "{%s}%s" % (self._cbc, 'Percent'),
                                     nsmap={'cbc': self._cbc}).text = detalle.tipISC
                else:
                    if tributo.ideTributo not in ['7152']:
                        etree.SubElement(tax_category, "{%s}%s" % (self._cbc, 'TaxExemptionReasonCode'),
                                         listAgencyName="PE:SUNAT", listName="Afectacion del IGV",
                                         listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo07",
                                         nsmap={'cbc': self._cbc}).text = detalle.tipAfectacion
            tax_scheme = etree.SubElement(tax_category, "{%s}%s" % (self._cac, 'TaxScheme'), nsmap={'cac': self._cac})
            etree.SubElement(tax_scheme, "{%s}%s" % (self._cbc, 'ID'), schemeAgencyName="PE:SUNAT",
                             schemeName="Codigo de tributos",
                             schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo05",
                             nsmap={'cbc': self._cbc}).text = tributo.ideTributo
            etree.SubElement(tax_scheme, "{%s}%s" % (self._cbc, 'Name'),
                             nsmap={'cbc': self._cbc}).text = tributo.nomTributo
            etree.SubElement(tax_scheme, "{%s}%s" % (self._cbc, 'TaxTypeCode'),
                             nsmap={'cbc': self._cbc}).text = tributo.codTipTributo
    ...
    def _getDetalle(self):
        ...
        for detalle in self.documento.detalles:
            ...
            etree.SubElement(line, "{%s}%s" % (self._cbc, 'LineExtensionAmount'), currencyID=self.documento.tipMoneda,
                             nsmap={'cbc': self._cbc}).text = str(detalle.mtoValorVentaItem)

            # Precio de venta unitario por ítem
            reference = etree.SubElement(line, "{%s}%s" % (self._cac, 'PricingReference'), nsmap={'cac': self._cac})
            condition_price = etree.SubElement(reference, "{%s}%s" % (self._cac, 'AlternativeConditionPrice'),
                                               nsmap={'cac': self._cac})
            if detalle.mtoValorReferencialUnitario == 0.0:
                etree.SubElement(condition_price, "{%s}%s" % (self._cbc, 'PriceAmount'),
                                 currencyID=self.documento.tipMoneda, nsmap={'cbc': self._cbc}).text = str(
                    detalle.mtoPrecioVentaUnitario)
                etree.SubElement(condition_price, "{%s}%s" % (self._cbc, 'PriceTypeCode'), listName="Tipo de Precio",
                                 listAgencyName="PE:SUNAT",
                                 listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo16",
                                 nsmap={'cbc': self._cbc}).text = "01"
            else:
            ...
            # Detalle
            etree.SubElement(item, "{%s}%s" % (self._cbc, 'Description'), nsmap={'cbc': self._cbc}).text = etree.CDATA(
                detalle.descripcion.replace("\n", " ")[:250])

            # Código de producto
            identification = etree.SubElement(item, "{%s}%s" % (self._cac, 'SellersItemIdentification'),
                                              nsmap={'cac': self._cac})
            etree.SubElement(identification, "{%s}%s" % (self._cbc, 'ID'),
                             nsmap={'cbc': self._cbc}).text = detalle.codProducto

            # Código de producto SUNAT
            if detalle.codProductoSUNAT:
                classification = etree.SubElement(item, "{%s}%s" % (self._cac, 'CommodityClassification'),
                                                  nsmap={'cac': self._cac})
                etree.SubElement(classification, "{%s}%s" % (self._cbc, 'ItemClassificationCode'), listID="UNSPSC",
                                 listAgencyName="GS1 US",
                                 listName="Item Classification",
                                 nsmap={'cbc': self._cbc}).text = detalle.codProductoSUNAT

            # codigo GTIN
            if detalle.codProductoGTIN and detalle.tipoProductoGTIN:
                standard = etree.SubElement(item, "{%s}%s" % (self._cac, 'StandardItemIdentification'),
                                            nsmap={'cac': self._cac})
                etree.SubElement(standard, "{%s}%s" % (self._cbc, 'ItemClassificationCode'),
                                 schemeID=detalle.tipoProductoGTIN,
                                 nsmap={'cbc': self._cbc}).text = detalle.codProductoGTIN

            if detalle.placa:
                identification = etree.SubElement(item, "{%s}%s" % (self._cac, 'AdditionalItemProperty'),
                                                  nsmap={'cac': self._cac})
                etree.SubElement(identification, "{%s}%s" % (self._cbc, 'Name'),
                                 nsmap={'cbc': self._cbc}).text = 'Gastos Art. 37 Renta: Número de Placa'
                etree.SubElement(identification, "{%s}%s" % (self._cbc, 'NameCode'), listName='Propiedad del item',
                                 listAgencyName='PE:SUNAT',
                                 listURI='urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo55',
                                 nsmap={'cbc': self._cbc}).text = '7000'
                etree.SubElement(identification, "{%s}%s" % (self._cbc, 'Value'),
                                 nsmap={'cbc': self._cbc}).text = detalle.placa

            # Valor unitario por ítem
            price = etree.SubElement(line, "{%s}%s" % (self._cac, 'Price'), nsmap={'cac': self._cac})
            etree.SubElement(price, "{%s}%s" % (self._cbc, 'PriceAmount'), currencyID=self.documento.tipMoneda,
                             nsmap={'cbc': self._cbc}).text = str(detalle.mtoValorUnitario)
    ...
    def _getUbl(self, customization='1.0'):
        if self.documento.tipoDocumento == '09':
            ...
            etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'CustomizationID'),
                             nsmap={'cbc': self._cbc}).text = '2.0'
        else:
            etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'UBLVersionID'), nsmap={'cbc': self._cbc}).text = '2.1'
            etree.SubElement(self._root, "{%s}%s" % (self._cbc, 'CustomizationID'), schemeAgencyName='PE:SUNAT',
                             nsmap={'cbc': self._cbc}).text = '2.0'

    def _getFactura(self):
        if self.documento.tipoDocumento in ['07']:
            nsmap = self._nsmap("CreditNote-2")
            self._root = etree.Element("{urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2}CreditNote",
                                       nsmap=nsmap)
        elif self.documento.tipoDocumento in ['08']:
            nsmap = self._nsmap("DebitNote-2")
            self._root = etree.Element("{urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2}DebitNote",
                                       nsmap=nsmap)
        else:
    ...
    def _getMotivoNota(self):
        ...
        if self.documento.tipoDocumento in ['08']:
            etree.SubElement(discrepancy, tag.text, listAgencyName="PE:SUNAT",
                             ...
                             nsmap={'cbc': tag.namespace}).text = self.documento.codigoNota
    ...
    def _getDocumentosModificados(self, documentosModificados, detalle=False):
        for documentoModificado in documentosModificados:
            ...
            etree.SubElement(invoice, tag.text, listAgencyName='PE:SUNAT', listName='Tipo de Documento',
                             listURI='urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo01',
                             nsmap={'cbc': tag.namespace}).text = documentoModificado.tipoDocumento
    ...
    def _getGuia(self):
        ...
        if self.documento.modoTraslado == '01':
            ...
            etree.SubElement(event, tag.text, nsmap={'cbc': tag.namespace}).text = self.documento.fechaInicioTraslado or self.documento.fechaTraslado
        else:
            ...
            for transportista in self.documento.transportistas:
                ...
                customer = etree.SubElement(stage, tag.text, nsmap={'cac': tag.namespace})
                tag = etree.QName(self._cbc, 'ID')
                etree.SubElement(customer, tag.text, schemeID=transportista.tipoDocumento or '-',
                                 nsmap={'cbc': tag.namespace}).text = transportista.numDocumento or '-'

                tag = etree.QName(self._cbc, 'FirstName')
                etree.SubElement(customer, tag.text, nsmap={'cbc': tag.namespace}).text = transportista.apPaterno or '-'
                tag = etree.QName(self._cbc, 'FamilyName')
                etree.SubElement(customer, tag.text,
                                 nsmap={'cbc': tag.namespace}).text = transportista.nomPersona or '-'
                ...
                if transportista.licencia:
                    ...
                    reference = etree.SubElement(customer, tag.text, nsmap={'cac': tag.namespace})
                    tag = etree.QName(self._cbc, 'ID')
                    etree.SubElement(reference, tag.text,
                                     nsmap={'cbc': tag.namespace}).text = transportista.licencia or '-'
        ...
        if self.documento.modoTraslado == '02':
            ...
            tag = etree.QName(self._cbc, 'ID')
            etree.SubElement(transport, tag.text, nsmap={'cbc': tag.namespace}).text = self.documento.placa.replace("-",
                                                                                                                    "")
            ...
            for vehículo in self.documento.vehículos:
                ...
                tag = etree.QName(self._cbc, 'ID')
                etree.SubElement(equipment, tag.text, nsmap={'cbc': tag.namespace}).text = vehículo.placa.replace("-",
                                                                                                                  "")
        ...
        tag = etree.QName(self._cbc, 'Line')
        etree.SubElement(oaddressline, tag.text, nsmap={'cbc': tag.namespace}).text = self.documento.direccionPartida
        ...
        for detalle in self.documento.detalles:
            ...
            tag = etree.QName(self._cbc, 'Description')
            etree.SubElement(item, tag.text, nsmap={'cbc': tag.namespace}).text = etree.CDATA(
                detalle.descripcion.replace("\n", " ")[:250])
