...
class Ubl21:
    ...
    def _getX509Template(self):
        ...
        signature = etree.SubElement(
            content, tag.text, Id="signatureGrupoYACCK", nsmap={"ds": tag.namespace}
        )
    ...
    def _getFirma(self):
        ...
        ).text = self.documento.emisor.numDocumento
        ...
        etree.SubElement(name, tag.text, nsmap={"cbc": tag.namespace}).text = (
            self.documento.emisor.nombre
        )
        ...
        etree.SubElement(reference, tag.text, nsmap={"cbc": tag.namespace}).text = (
            "#signatureGrupoYACCK"
        )
    ...
    def _nsmap(self, documento):
        nsmap = OrderedDict(
            [
                (None, "urn:oasis:names:specification:ubl:schema:xsd:%s" % documento),
                ("cac", self._cac),
                ("cbc", self._cbc),
                ("ccts", self._ccts),
                ("ds", self._ds),
    ...
    def _getDocumento(self):
        etree.SubElement(
            self._root, "{%s}%s" % (self._cbc, "ID"), nsmap={"cbc": self._cbc}
        ).text = (self.documento.numero or "")
        etree.SubElement(
            self._root, "{%s}%s" % (self._cbc, "IssueDate"), nsmap={"cbc": self._cbc}
        ).text = self.documento.fecEmision.strftime("%Y-%m-%d")
        etree.SubElement(
            self._root, "{%s}%s" % (self._cbc, "IssueTime"), nsmap={"cbc": self._cbc}
        ).text = self.documento.fecEmision.strftime("%H:%M:%S")
        if self.documento.tipoDocumento in ["01", "03"]:
            if self.documento.fecVencimiento:
                etree.SubElement(
                    self._root,
                    "{%s}%s" % (self._cbc, "DueDate"),
                    nsmap={"cbc": self._cbc},
                ).text = (
                    self.documento.fecVencimiento or ""
                )
            etree.SubElement(
                self._root,
                "{%s}%s" % (self._cbc, "InvoiceTypeCode"),
                listID=self.documento.tipOperacion,
                listAgencyName="PE:SUNAT",
                listName="Tipo de Documento",
                listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo01",
                nsmap={"cbc": self._cbc},
            ).text = self.documento.tipoDocumento
        if self.documento.tipoDocumento in ["09"]:
            ...
            if self.documento.observacion:
                ...
                etree.SubElement(
                    self._root, tag.text, nsmap={"cbc": tag.namespace}
                ).text = (self.documento.observacion or "-")
        # Leyenda
        if self.documento.tipoDocumento not in ["09"]:
            for leyenda in self.documento.leyendas:
                ...
                etree.SubElement(
                    self._root,
                    tag.text,
                    languageLocaleID=leyenda.codLeyenda,
                    nsmap={"cbc": tag.namespace},
                ).text = leyenda.desLeyenda

            etree.SubElement(
                self._root,
                "{%s}%s" % (self._cbc, "DocumentCurrencyCode"),
                listID="ISO 4217 Alpha",
                listName="Currency",
                listAgencyName="United Nations Economic Commission for Europe",
                nsmap={"cbc": self._cbc},
            ).text = self.documento.tipMoneda

            tag = etree.QName(self._cbc, "LineCountNumeric")
            etree.SubElement(
                self._root, tag.text, nsmap={"cbc": tag.namespace}
            ).text = str(len(self.documento.detalles))

            if self.documento.referencia and self.documento.tipoDocumento in [
                "01",
                "03",
            ]:
                ref = (
                    self.documento.referencia.replace(" ", "")
                    .replace("-", "")
                    .replace("/", "")
                )
                if ref and re.match(r"^[a-zA-Z0-9]*$", ref):
                    ...
                    tag = etree.QName(self._cbc, "ID")
                    etree.SubElement(
                        order, tag.text, nsmap={"cbc": tag.namespace}
                    ).text = ref
            if self.documento.tipoDocumento in ["01", "03"]:
                for guia in self.documento.guias:
                    despatch = etree.SubElement(
                        self._root,
                        "{%s}%s" % (self._cac, "DespatchDocumentReference"),
                        nsmap={"cac": self._cac},
                    )
                    etree.SubElement(
                        despatch, "{%s}%s" % (self._cbc, "ID"), nsmap={"cbc": self._cbc}
                    ).text = guia.numero
                    etree.SubElement(
                        despatch,
                        "{%s}%s" % (self._cbc, "DocumentTypeCode"),
                        listAgencyName="PE:SUNAT",
                        listName="Tipo de Documento",
                        listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo01",
                        nsmap={"cbc": self._cbc},
                    ).text = guia.tipoDocumento

    def _getDetraccion(self):
        if self.documento.detraccion:
            payment = etree.SubElement(
                self._root,
                "{%s}%s" % (self._cac, "PaymentMeans"),
                nsmap={"cac": self._cac},
            )
            etree.SubElement(
                payment, "{%s}%s" % (self._cbc, "ID"), nsmap={"cbc": self._cbc}
            ).text = "Detraccion"
            etree.SubElement(
                payment,
                "{%s}%s" % (self._cbc, "PaymentMeansCode"),
                nsmap={"cbc": self._cbc},
            ).text = self.documento.detraccion.medioPago

            payee = etree.SubElement(
                payment,
                "{%s}%s" % (self._cac, "PayeeFinancialAccount"),
                nsmap={"cac": self._cac},
            )
            etree.SubElement(
                payee, "{%s}%s" % (self._cbc, "ID"), nsmap={"cbc": self._cbc}
            ).text = self.documento.detraccion.cuentaBanco

            payment = etree.SubElement(
                self._root,
                "{%s}%s" % (self._cac, "PaymentTerms"),
                nsmap={"cac": self._cac},
            )
            etree.SubElement(
                payment, "{%s}%s" % (self._cbc, "ID"), nsmap={"cbc": self._cbc}
            ...
            etree.SubElement(
                payment,
                "{%s}%s" % (self._cbc, "PaymentMeansID"),
                nsmap={"cbc": self._cbc},
            ).text = self.documento.detraccion.codigo

            etree.SubElement(
                payment,
                "{%s}%s" % (self._cbc, "PaymentPercent"),
                nsmap={"cbc": self._cbc},
            ).text = str(self.documento.detraccion.porcentaje)

            etree.SubElement(
                payment,
                "{%s}%s" % (self._cbc, "Amount"),
                currencyID="PEN",
                nsmap={"cbc": self._cbc},
            ).text = str(self.documento.detraccion.monto)
    ...
    def _getRetencion(self):
        if self.documento.retencion:
            ...
            etree.SubElement(
                allowance_charge, tag.text, nsmap={"cbc": tag.namespace}
            ).text = ("%.5f" % self.documento.retencion.porcentaje)

            tag = etree.QName(self._cbc, "Amount")
            etree.SubElement(
                allowance_charge,
                tag.text,
                currencyID=self.documento.tipMoneda,
                nsmap={"cbc": tag.namespace},
            ).text = str(self.documento.retencion.monto)
            ...
            etree.SubElement(
                ...
                currencyID=self.documento.tipMoneda,
                nsmap={"cbc": tag.namespace},
            ).text = str(self.documento.retencion.base)

    def _getMedioPago(self):
        if (
            self.documento.tipoDocumento in ["07"]
            and self.documento.medioPago.tipo == "Credito"
        ) or self.documento.tipoDocumento not in ["07"]:
            payment = etree.SubElement(
                self._root,
                "{%s}%s" % (self._cac, "PaymentTerms"),
                nsmap={"cac": self._cac},
            ...
            etree.SubElement(
                ...
                "{%s}%s" % (self._cbc, "PaymentMeansID"),
                nsmap={"cbc": self._cbc},
            ).text = self.documento.medioPago.tipo
        if self.documento.medioPago.tipo == "Credito":
            etree.SubElement(
                ...
                currencyID=self.documento.tipMoneda,
                nsmap={"cbc": self._cbc},
            ).text = str(self.documento.medioPago.monto)
            i = 1
            for cuota in self.documento.medioPago.cuotas:
                ...
                etree.SubElement(
                    ...
                    currencyID=self.documento.tipMoneda,
                    nsmap={"cbc": self._cbc},
                ).text = str(cuota.monto)
                etree.SubElement(
                    payment,
                    "{%s}%s" % (self._cbc, "PaymentDueDate"),
                    nsmap={"cbc": self._cbc},
                ).text = str(cuota.fecha)
                i += 1
    ...
    def _getEmisor(self):
        ...
        if self.documento.tipoDocumento in ["09"]:
            ...
            identification = etree.SubElement(
                party,
                "{%s}%s" % (self._cac, "PartyIdentification"),
                nsmap={"cac": self._cac},
            )
            etree.SubElement(
                identification,
            ...
            ).text = etree.CDATA(emisor.nombre)
        else:
            supplier = etree.SubElement(
                self._root,
                "{%s}%s" % (self._cac, "AccountingSupplierParty"),
                nsmap={"cac": self._cac},
            )
            party = etree.SubElement(
                supplier, "{%s}%s" % (self._cac, "Party"), nsmap={"cac": self._cac}
            )
            identification = etree.SubElement(
                party,
                "{%s}%s" % (self._cac, "PartyIdentification"),
            ...
            if emisor.nomComercial and emisor.nomComercial != "-":
                name = etree.SubElement(
                    party, "{%s}%s" % (self._cac, "PartyName"), nsmap={"cac": self._cac}
                )
                etree.SubElement(
                    name, "{%s}%s" % (self._cbc, "Name"), nsmap={"cbc": self._cbc}
                ).text = etree.CDATA(emisor.nomComercial)
            entity = etree.SubElement(
                party,
                "{%s}%s" % (self._cac, "PartyLegalEntity"),
                nsmap={"cac": self._cac},
            )
            etree.SubElement(
                entity,
                "{%s}%s" % (self._cbc, "RegistrationName"),
                nsmap={"cbc": self._cbc},
            ).text = etree.CDATA(emisor.nombre)

            address = etree.SubElement(
                entity,
                "{%s}%s" % (self._cac, "RegistrationAddress"),
                nsmap={"cac": self._cac},
            )
    ...
    def _getAdquirente(self, detalle=None, adquirente=None, tercero=None):
        if self.documento.tipoDocumento in ["09"]:
            destinatario = (
                tercero
                and self.documento.establecimientoTercero
                or self.documento.destinatario
            )
            if tercero:
            ...
            etree.SubElement(
                customer,
                tag.text,
                schemeID=destinatario.tipoDocumento or "-",
                nsmap={"cbc": tag.namespace},
            ).text = (
                destinatario.numDocumento or "-"
            )
            tag = etree.QName(self._cac, "Party")
            ...
            etree.SubElement(
                identification,
                "{%s}%s" % (self._cbc, "ID"),
                schemeID=destinatario.tipoDocumento,
                schemeName="Documento de Identidad",
                schemeAgencyName="PE:SUNAT",
                schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06",
                nsmap={"cbc": self._cbc},
            ).text = destinatario.numDocumento
            tag = etree.QName(self._cac, "PartyLegalEntity")
            entity = etree.SubElement(party, tag.text, nsmap={"cac": tag.namespace})
            tag = etree.QName(self._cbc, "RegistrationName")
            etree.SubElement(entity, tag.text, nsmap={"cbc": tag.namespace}).text = (
                etree.CDATA(destinatario.nombre)
            )
        else:
            adquirente = adquirente or self.documento.adquirente
            supplier = etree.SubElement(
                self._root,
                "{%s}%s" % (self._cac, "AccountingCustomerParty"),
                nsmap={"cac": self._cac},
            )
            party = etree.SubElement(
                supplier, "{%s}%s" % (self._cac, "Party"), nsmap={"cac": self._cac}
            ...
            etree.SubElement(
                identification,
                "{%s}%s" % (self._cbc, "ID"),
                schemeID=adquirente.tipoDocumento,
                schemeName="Documento de Identidad",
                schemeAgencyName="PE:SUNAT",
                schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06",
                nsmap={"cbc": self._cbc},
            ).text = adquirente.numDocumento
            entity = etree.SubElement(
                party,
                "{%s}%s" % (self._cac, "PartyLegalEntity"),
                nsmap={"cac": self._cac},
            ...
            etree.SubElement(
                ...
                "{%s}%s" % (self._cbc, "RegistrationName"),
                nsmap={"cbc": self._cbc},
            ).text = etree.CDATA(adquirente.nombre)
    ...
    # Falta Información adicional - Datos del sujeto que realiza la operación
    # por cuenta del adquirente o usuario.
        ...
        if self.documento.tipoDocumento in ["09"]:
            for relacionado in self.documento.documentosRelacionados:
                ...
                etree.SubElement(
                    ...
                    nsmap={"cbc": tag.namespace},
                ).text = relacionado.tipoDocumento
                tag = etree.QName(self._cac, "IssuerParty")
                issuer_party = etree.SubElement(
                    reference, tag.text, nsmap={"cac": tag.namespace}
                )
                tag = etree.QName(self._cac, "PartyIdentification")
                party = etree.SubElement(
                    issuer_party, tag.text, nsmap={"cac": tag.namespace}
                )
                if relacionado.emisor:
                    ...
                    etree.SubElement(
                        party,
                        tag.text,
                        schemeID=relacionado.emisor.tipoDocumento,
                        schemeName="Documento de Identidad",
                        schemeAgencyName="PE:SUNAT",
                        schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06",
                        nsmap={"cbc": tag.namespace},
                    ).text = relacionado.emisor.numDocumento
        ...
        if self.documento.tipoDocumento in ["01", "03"]:
            ...
            for anticipo in self.documento.anticipos:
                ...
                etree.SubElement(
                    additional,
                    tag.text,
                    schemeID=self.documento.tipoDocumento,
                    nsmap={"cbc": tag.namespace},
                ).text = anticipo.numero
                ...
                tag = etree.QName(self._cac, "IssuerParty")
                issuer_party = etree.SubElement(
                    additional, tag.text, nsmap={"cac": tag.namespace}
                )
                tag = etree.QName(self._cac, "PartyIdentification")
                party = etree.SubElement(
                    issuer_party, tag.text, nsmap={"cac": tag.namespace}
                ...
                etree.SubElement(
                    party,
                    tag.text,
                    schemeID=self.documento.emisor.tipoDocumento,
                    schemeName="Documento de Identidad",
                    schemeAgencyName="PE:SUNAT",
                    schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06",
                    nsmap={"cbc": tag.namespace},
    ...
    def _getCargoDescuentos(self, root, cargoDescuentos, detalle=False):
        for cargoDescuento in cargoDescuentos:
            tag = etree.QName(self._cac, "AllowanceCharge")
            allowance_charge = etree.SubElement(
                root, tag.text, nsmap={"cac": tag.namespace}
            )
            tag = etree.QName(self._cbc, "ChargeIndicator")
            etree.SubElement(
                allowance_charge, tag.text, nsmap={"cbc": tag.namespace}
            ).text = cargoDescuento.indicador

            tag = etree.QName(self._cbc, "AllowanceChargeReasonCode")
            if detalle:
                etree.SubElement(
                    allowance_charge, tag.text, nsmap={"cbc": tag.namespace}
                ).text = cargoDescuento.codigo  # Verificar cattalogo 53
            else:
                etree.SubElement(
                    allowance_charge,
                    tag.text,
            ...
            etree.SubElement(
                allowance_charge, tag.text, nsmap={"cbc": tag.namespace}
            ).text = ("%.5f" % cargoDescuento.porcentaje)
            ...
            etree.SubElement(
                ...
                currencyID=self.documento.tipMoneda,
                nsmap={"cbc": tag.namespace},
            ).text = str(cargoDescuento.base)

    def _getTributos(self, root, total, detalle=None, tributos=None):
        tax_total = etree.SubElement(
            root, "{%s}%s" % (self._cac, "TaxTotal"), nsmap={"cac": self._cac}
        )
        etree.SubElement(
            tax_total,
            "{%s}%s" % (self._cbc, "TaxAmount"),
            currencyID=self.documento.tipMoneda,
            nsmap={"cbc": self._cbc},
        ...
        for tributo in tributos:
            tax_subtotal = etree.SubElement(
                tax_total,
                "{%s}%s" % (self._cac, "TaxSubtotal"),
                nsmap={"cac": self._cac},
            )
            if detalle and tributo.ideTributo not in ["7152"]:
                etree.SubElement(
                    tax_subtotal,
                    "{%s}%s" % (self._cbc, "TaxableAmount"),
                    currencyID=self.documento.tipMoneda,
                    nsmap={"cbc": self._cbc},
                ).text = str(round(tributo.baseTributo, 2))
            elif not detalle:
            ...
            etree.SubElement(
                ...
                currencyID=self.documento.tipMoneda,
                nsmap={"cbc": self._cbc},
            ).text = str(round(tributo.montoTributo, 2))
            if detalle and tributo.ideTributo in ["7152"]:
                etree.SubElement(
                    tax_subtotal,
                    "{%s}%s" % (self._cbc, "BaseUnitMeasure"),
                    unitCode="NIU",
                    nsmap={"cbc": self._cbc},
                ).text = str(tributo.cantidad)
            tax_category = etree.SubElement(
                tax_subtotal,
                "{%s}%s" % (self._cac, "TaxCategory"),
                nsmap={"cac": self._cac},
            )
            if detalle:
                if tributo.ideTributo in ["7152"]:
                    etree.SubElement(
                        tax_category,
                        "{%s}%s" % (self._cbc, "Percent"),
                        nsmap={"cbc": self._cbc},
                    ).text = str(0.0)
                    etree.SubElement(
                        tax_category,
                        "{%s}%s" % (self._cbc, "PerUnitAmount"),
                        currencyID=self.documento.tipMoneda,
                        nsmap={"cbc": self._cbc},
                    ).text = str(tributo.procentaje)
                else:
                    etree.SubElement(
                        tax_category,
                        "{%s}%s" % (self._cbc, "Percent"),
                ...
                if tributo.ideTributo == "2000":
                    ...
                    if tributo.ideTributo not in ["7152"]:
                        etree.SubElement(
                            tax_category,
                            "{%s}%s" % (self._cbc, "TaxExemptionReasonCode"),
                            listAgencyName="PE:SUNAT",
                            listName="Afectacion del IGV",
                            listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo07",
                            nsmap={"cbc": self._cbc},
                        ).text = detalle.tipAfectacion
            tax_scheme = etree.SubElement(
                tax_category,
                "{%s}%s" % (self._cac, "TaxScheme"),
                nsmap={"cac": self._cac},
            )
            etree.SubElement(
                tax_scheme,
                ...
                nsmap={"cbc": self._cbc},
            ).text = tributo.ideTributo
            etree.SubElement(
                tax_scheme, "{%s}%s" % (self._cbc, "Name"), nsmap={"cbc": self._cbc}
            ).text = tributo.nomTributo
            etree.SubElement(
                tax_scheme,
                "{%s}%s" % (self._cbc, "TaxTypeCode"),
                nsmap={"cbc": self._cbc},
            ).text = tributo.codTipTributo
    ...
    def _getMontosGlobales(self):
        ...
        etree.SubElement(
            ...
            currencyID=self.documento.tipMoneda,
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.totalValVenta)
        ...
        etree.SubElement(
            ...
            currencyID=self.documento.tipMoneda,
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.totalImpVenta)
        ...
        etree.SubElement(
            ...
            currencyID=self.documento.tipMoneda,
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.totalDescuentos)
        ...
        etree.SubElement(
            ...
            currencyID=self.documento.tipMoneda,
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.totalCargos)
        ...
        etree.SubElement(
            ...
            currencyID=self.documento.tipMoneda,
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.totalAnticipos)
        ...
        etree.SubElement(
            ...
            currencyID=self.documento.tipMoneda,
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.totalVenta)
    ...
    def _getDetalle(self):
        ...
        for detalle in self.documento.detalles:
            ...
            line = etree.SubElement(
                self._root, "{%s}%s" % (self._cac, tipo_linea), nsmap={"cac": self._cac}
            )
            etree.SubElement(
                line, "{%s}%s" % (self._cbc, "ID"), nsmap={"cbc": self._cbc}
            ).text = str(cont)
            cont += 1
            if self.documento.tipoDocumento == "08":
            ...
            etree.SubElement(
                line,
                "{%s}%s" % (self._cbc, cantidad_linea),
                unitCode=detalle.codUnidadMedida,
                unitCodeListID="UN/ECE rec 20",
                unitCodeListAgencyName="United Nations Economic Commission for Europe",
                nsmap={"cbc": self._cbc},
            ).text = str(detalle.cantidad)
            # Valor de venta por ítem
            etree.SubElement(
                line,
                "{%s}%s" % (self._cbc, "LineExtensionAmount"),
                currencyID=self.documento.tipMoneda,
                nsmap={"cbc": self._cbc},
            ).text = str(detalle.mtoValorVentaItem)

            # Precio de venta unitario por ítem
            reference = etree.SubElement(
                line,
                "{%s}%s" % (self._cac, "PricingReference"),
                nsmap={"cac": self._cac},
            )
            condition_price = etree.SubElement(
                reference,
                "{%s}%s" % (self._cac, "AlternativeConditionPrice"),
                nsmap={"cac": self._cac},
            )
            if detalle.mtoValorReferencialUnitario == 0.0:
                etree.SubElement(
                    condition_price,
                    "{%s}%s" % (self._cbc, "PriceAmount"),
                    currencyID=self.documento.tipMoneda,
                    nsmap={"cbc": self._cbc},
                ).text = str(detalle.mtoPrecioVentaUnitario)
                etree.SubElement(
                    condition_price,
                    "{%s}%s" % (self._cbc, "PriceTypeCode"),
                ...
                # Valor referencial unitario por ítem en operaciones gratuitas (no
                # onerosas)
                etree.SubElement(
                    condition_price,
                    "{%s}%s" % (self._cbc, "PriceAmount"),
                    currencyID=self.documento.tipMoneda,
                    nsmap={"cbc": self._cbc},
                ).text = str(detalle.mtoValorReferencialUnitario)
                etree.SubElement(
                    condition_price,
                    "{%s}%s" % (self._cbc, "PriceTypeCode"),
                    listName="Tipo de Precio",
            ...
            self._getTributos(
                line, detalle.sumTotTributosItem, detalle, detalle.tributos
            )

            item = etree.SubElement(
                line, "{%s}%s" % (self._cac, "Item"), nsmap={"cac": self._cac}
            )
            # Detalle
            etree.SubElement(
                item, "{%s}%s" % (self._cbc, "Description"), nsmap={"cbc": self._cbc}
            ).text = etree.CDATA(detalle.descripcion.replace("\n", " ")[:250])

            # Código de producto
            identification = etree.SubElement(
                item,
                "{%s}%s" % (self._cac, "SellersItemIdentification"),
                nsmap={"cac": self._cac},
            )
            etree.SubElement(
                identification, "{%s}%s" % (self._cbc, "ID"), nsmap={"cbc": self._cbc}
            ).text = detalle.codProducto

            # Código de producto SUNAT
            if detalle.codProductoSUNAT:
                classification = etree.SubElement(
                    item,
                    "{%s}%s" % (self._cac, "CommodityClassification"),
                    nsmap={"cac": self._cac},
                )
                etree.SubElement(
                    classification,
                    "{%s}%s" % (self._cbc, "ItemClassificationCode"),
                    listID="UNSPSC",
                    listAgencyName="GS1 US",
                    listName="Item Classification",
                    nsmap={"cbc": self._cbc},
                ).text = detalle.codProductoSUNAT

            # codigo GTIN
            if detalle.codProductoGTIN and detalle.tipoProductoGTIN:
                standard = etree.SubElement(
                    item,
                    "{%s}%s" % (self._cac, "StandardItemIdentification"),
                    nsmap={"cac": self._cac},
                )
                etree.SubElement(
                    standard,
                    "{%s}%s" % (self._cbc, "ItemClassificationCode"),
                    schemeID=detalle.tipoProductoGTIN,
                    nsmap={"cbc": self._cbc},
                ).text = detalle.codProductoGTIN

            if detalle.placa:
                identification = etree.SubElement(
                    item,
                    "{%s}%s" % (self._cac, "AdditionalItemProperty"),
                    nsmap={"cac": self._cac},
                )
                etree.SubElement(
                    identification,
            ...
            price = etree.SubElement(
                line, "{%s}%s" % (self._cac, "Price"), nsmap={"cac": self._cac}
            )
            etree.SubElement(
                price,
                "{%s}%s" % (self._cbc, "PriceAmount"),
                currencyID=self.documento.tipMoneda,
                nsmap={"cbc": self._cbc},
            ).text = str(detalle.mtoValorUnitario)

    def _getUbl(self, customization="1.0"):
        if self.documento.tipoDocumento == "09":
            etree.SubElement(
                self._root,
                "{%s}%s" % (self._cbc, "UBLVersionID"),
                nsmap={"cbc": self._cbc},
            ).text = "2.1"
            etree.SubElement(
                self._root,
                "{%s}%s" % (self._cbc, "CustomizationID"),
                nsmap={"cbc": self._cbc},
            ).text = "2.0"
        else:
            etree.SubElement(
    ...
    def _getMotivoNota(self):
        ...
        if self.documento.tipoDocumento in ["08"]:
            etree.SubElement(
                ...
                nsmap={"cbc": tag.namespace},
            ).text = self.documento.codigoNota
        ...
        etree.SubElement(discrepancy, tag.text, nsmap={"cbc": tag.namespace}).text = (
            etree.CDATA(self.documento.motivo)
        )
    ...
    def _getDocumentosModificados(self, documentosModificados, detalle=False):
        for documentoModificado in documentosModificados:
            ...
            tag = etree.QName(self._cbc, "ID")
            etree.SubElement(invoice, tag.text, nsmap={"cbc": tag.namespace}).text = (
                documentoModificado.numero
            )
            tag = etree.QName(self._cbc, "DocumentTypeCode")
            etree.SubElement(
                invoice,
                ...
                listURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo01",
                nsmap={"cbc": tag.namespace},
            ).text = documentoModificado.tipoDocumento
    ...
    def _getGuia(self):
        ...
        etree.SubElement(shipment, tag.text, nsmap={"cbc": tag.namespace}).text = (
            self.documento.motivo
        )
        if self.documento.descripcion:
            tag = etree.QName(self._cbc, "HandlingInstructions")
            etree.SubElement(shipment, tag.text, nsmap={"cbc": tag.namespace}).text = (
                etree.CDATA(self.documento.descripcion)
            )
        tag = etree.QName(self._cbc, "GrossWeightMeasure")
        etree.SubElement(
            shipment,
            tag.text,
            unitCode=self.documento.pesoBrutoUnidad or "KGM",
            nsmap={"cbc": tag.namespace},
        ).text = str(self.documento.pesoBruto)
        if self.documento.motivo in ["08", "09"]:
            tag = etree.QName(self._cbc, "TotalTransportHandlingUnitQuantity")
            etree.SubElement(shipment, tag.text, nsmap={"cbc": tag.namespace}).text = (
                str(self.documento.bultos)
            )
        ...
        etree.SubElement(stage, tag.text, nsmap={"cbc": tag.namespace}).text = (
            self.documento.modoTraslado
        )
        ...
        etree.SubElement(period, tag.text, nsmap={"cbc": tag.namespace}).text = (
            self.documento.fechaTraslado
        )
        ...
        if self.documento.modoTraslado == "01":
            for transportista in self.documento.transportistas:
                ...
                customer = etree.SubElement(
                    stage, tag.text, nsmap={"cac": tag.namespace}
                )
                tag = etree.QName(self._cac, "PartyIdentification")
                ident = etree.SubElement(
                    customer, tag.text, nsmap={"cac": tag.namespace}
                )
                tag = etree.QName(self._cbc, "ID")
                etree.SubElement(
                    ident,
                    tag.text,
                    schemeID=transportista.tipoDocumento or "-",
                    nsmap={"cbc": tag.namespace},
                ).text = (
                    transportista.numDocumento or "-"
                )
                ...
                tag = etree.QName(self._cbc, "RegistrationName")
                etree.SubElement(party, tag.text, nsmap={"cbc": tag.namespace}).text = (
                    etree.CDATA(transportista.nombre)
                )
                if transportista.numRegistro:
                    tag = etree.QName(self._cbc, "CompanyID")
                    etree.SubElement(
                        party, tag.text, nsmap={"cbc": tag.namespace}
                    ).text = transportista.numRegistro
                break
        else:
            ...
            etree.SubElement(road, tag.text, nsmap={"cbc": tag.namespace}).text = (
                self.documento.placa
            )
            principal = True
            for transportista in self.documento.transportistas:
                ...
                ).text = (transportista.apPaterno or "-")
                tag = etree.QName(self._cbc, "FamilyName")
                etree.SubElement(
                    customer, tag.text, nsmap={"cbc": tag.namespace}
                ).text = (transportista.nomPersona or "-")
                ...
                if transportista.licencia:
                    ...
                    etree.SubElement(
                        reference, tag.text, nsmap={"cbc": tag.namespace}
                    ).text = (transportista.licencia or "-")
        ...
        etree.SubElement(
            ...
            schemeName="Ubigeos",
            nsmap={"cbc": tag.namespace},
        ).text = self.documento.ubigeoLlegada

        if (
            self.documento.motivo in ["04"]
            and self.documento.codSucursalLlegada
            and self.documento.rucLlegada
        ):
            tag = etree.QName(self._cbc, "AddressTypeCode")
            etree.SubElement(
                address,
                tag.text,
                listID=self.documento.rucLlegada,
                nsmap={"cbc": tag.namespace},
            ).text = self.documento.codSucursalLlegada

        tag = etree.QName(self._cac, "AddressLine")
        oaddressline = etree.SubElement(address, tag.text, nsmap={"cac": tag.namespace})

        tag = etree.QName(self._cbc, "Line")
        etree.SubElement(oaddressline, tag.text, nsmap={"cbc": tag.namespace}).text = (
            self.documento.direccionLlegada
        )
        ...
        if self.documento.modoTraslado == "02":
            ...
            transport = etree.SubElement(
                shipment, tag.text, nsmap={"cac": tag.namespace}
            )
            tag = etree.QName(self._cbc, "ID")
            etree.SubElement(transport, tag.text, nsmap={"cbc": tag.namespace}).text = (
                self.documento.placa.replace("-", "")
            )

            for vehículo in self.documento.vehículos:
                # if self.documento.placa:
                tag = etree.QName(self._cac, "TransportEquipment")
                equipment = etree.SubElement(
                    transport, tag.text, nsmap={"cac": tag.namespace}
                )
                tag = etree.QName(self._cbc, "ID")
                etree.SubElement(
                    equipment, tag.text, nsmap={"cbc": tag.namespace}
                ).text = vehículo.placa.replace("-", "")

        # Aqui va datos del contenedor
        tag = etree.QName(self._cac, "Despatch")
        despatch = etree.SubElement(delivery, tag.text, nsmap={"cac": tag.namespace})
        ...
        etree.SubElement(
            ...
            schemeName="Ubigeos",
            nsmap={"cbc": tag.namespace},
        ).text = self.documento.ubigeoPartida

        if (
            self.documento.motivo in ["04"]
            and self.documento.codSucursalPartida
            and self.documento.rucPartida
        ):
            tag = etree.QName(self._cbc, "AddressTypeCode")
            etree.SubElement(
                oaddress,
                tag.text,
                listID=self.documento.rucPartida,
                nsmap={"cbc": tag.namespace},
            ).text = self.documento.codSucursalPartida

        tag = etree.QName(self._cac, "AddressLine")
        oaddressline = etree.SubElement(
            oaddress, tag.text, nsmap={"cac": tag.namespace}
        )

        tag = etree.QName(self._cbc, "Line")
        etree.SubElement(oaddressline, tag.text, nsmap={"cbc": tag.namespace}).text = (
            self.documento.direccionPartida
        )
        ...
        for detalle in self.documento.detalles:
            ...
            tag = etree.QName(self._cbc, "ID")
            etree.SubElement(despatch, tag.text, nsmap={"cbc": tag.namespace}).text = (
                str(cont)
            )
            ...
            etree.SubElement(ref, tag.text, nsmap={"cbc": tag.namespace}).text = str(
                cont
            )
            cont += 1
            ...
            tag = etree.QName(self._cbc, "Description")
            etree.SubElement(item, tag.text, nsmap={"cbc": tag.namespace}).text = (
                etree.CDATA(detalle.descripcion.replace("\n", " ")[:250])
            )

            tag = etree.QName(self._cac, "SellersItemIdentification")
            ident = etree.SubElement(item, tag.text, nsmap={"cac": tag.namespace})
            tag = etree.QName(self._cbc, "ID")
            etree.SubElement(ident, tag.text, nsmap={"cbc": tag.namespace}).text = (
                detalle.codProducto
            )
